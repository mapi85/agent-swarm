...
- **Providers LLM multiples** (onglet Réglages) : déclare autant de fournisseurs que nécessaire, de deux types — **Anthropic ou compatible** (Messages API ; URL/clé personnalisées, option « fonctionnalités natives » : thinking adaptatif, effort, compaction, cache) et **OpenAI ou compatible** (Chat Completions). Chaque provider déclare sa **liste de modèles** (et un modèle par défaut) ; un provider est **par défaut** ; chaque **agent est rattaché à un provider** et **choisit son modèle dans la liste de ce provider** (saisie manuelle possible). Les erreurs transitoires (429/5xx/réseau) sont **réessayées automatiquement**.
