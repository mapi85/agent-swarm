...
- **Providers LLM multiples** (onglet Réglages) : déclare autant de fournisseurs que nécessaire, de deux types — **Anthropic ou compatible** (Messages API ; URL/clé personnalisées, option « fonctionnalités natives » : thinking adaptatif, effort, compaction, cache) et **OpenAI ou compatible** (Chat Completions). La **liste des modèles est récupérée dynamiquement depuis l'API du provider** (endpoint `/v1/models`), avec repli sur une liste déclarée manuellement ; un provider est **par défaut** ; chaque **agent est rattaché à un provider** et **choisit son modèle dans la liste de ce provider** (rafraîchissable, saisie manuelle possible). Les erreurs transitoires (429/5xx/réseau) sont **réessayées automatiquement**.
