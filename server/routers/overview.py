...
def _scope(query, model_owner, user: User):
    return query if user.role == "admin" else query.where(model_owner == user.id)
...
async def overview(user: User = Depends(get_current_user), db: AsyncSession = Depends(get_db)):
    ...
    if user.role != "admin":
        agents_q = agents_q.where((Agent.owner_user_id == user.id) | (Agent.owner_user_id.is_(None)))
    ...
    running = await count(
        select(func.count()).select_from(Session).join(Task, Task.id == Session.task_id).where(
            ...
            *([] if user.role == "admin" else [Task.owner_user_id == user.id])))
    planned = await count(
        select(func.count()).select_from(Session).join(Task, Task.id == Session.task_id).where(
...
async def search(q: str, user: User = Depends(get_current_user), db: AsyncSession = Depends(get_db)):
    ...
    tasks_q = select(Task).where(or_(Task.title.ilike(like), Task.description.ilike(like))).limit(20)
    if user.role != "admin":
        tasks_q = tasks_q.where(Task.owner_user_id == user.id)
    missions_q = select(Mission).where(or_(Mission.title.ilike(like), Mission.mission.ilike(like))).limit(20)
    if user.role != "admin":
        missions_q = missions_q.where(Mission.owner_user_id == user.id)
    res_q = select(Resource).where(or_(Resource.name.ilike(like), Resource.description.ilike(like))).limit(20)
    if user.role != "admin":
        res_q = res_q.where(or_(Resource.owner_user_id == user.id, Resource.scope == "shared"))
